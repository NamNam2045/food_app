...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query(value = "SELECT r FROM Review r JOIN FETCH r.user WHERE r.restaurant.id = :rid",
           countQuery = "SELECT COUNT(r) FROM Review r WHERE r.restaurant.id = :rid")
    Page<Review> findByRestaurantForOwner(@Param("rid") Long restaurantId, Pageable pageable);
}
